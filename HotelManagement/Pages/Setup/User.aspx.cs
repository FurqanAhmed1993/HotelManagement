...
public partial class Pages_Setup_User : Base
{
    ...
    public void BindDropDown()
    {
        try
        {
            ...
            DataTable dtHotel = new BAL_HotelMaster().usp_HotelMaster_Crud(null);
            if (dtHotel.Rows.Count>0)
            {
                CommonObjects.BindCheckBoxList(chkHotelList, dtHotel, "HotelName", "HotelId", dtHotel.Rows.Count > 1 ? true : false, false);
            }
            else
            {
                Error("Please add Hotel(s)");
            }
        }
        catch (Exception ex)
        {
    ...
    public DataTable GetUserLogin()
    {
        ...
        string User = txtUserSearch.Text.Trim() == "" ? null : txtUserSearch.Text.Trim();
        ...
        dt = new BAL_User().usp_UserLogin_Get(0,User,null,null,null,null,_RoleId,true,0,null,null);
        return dt;
    }
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            IEnumerable<int> allChecked = (from item in chkHotelList.Items.Cast<ListItem>()
                                           where item.Selected && item.Value!="0"
                                           select int.Parse(item.Value));
            HotelIds = string.Join(",", allChecked);

            int Id = hfId.Value == string.Empty ? 0 : Convert.ToInt32(hfId.Value);
            if (Id == 0)
            {
                DataTable dt = new BAL_User().usp_UserLogin_Get(0,UserName,LoginId,GenericConstants.Password,EmailAddress,txtPhoneNumber.Text.Trim(),RoleID,chk_IsActive.Checked,UserId,UserIP,HotelIds,OperationType.Insert);
                if (dt != null && dt.Rows.Count > 0)
                {
            ...
            {
                DataTable dt = new BAL_User().usp_UserLogin_Get(Id, UserName, LoginId, GenericConstants.Password, EmailAddress, txtPhoneNumber.Text.Trim(), RoleID, chk_IsActive.Checked, UserId, UserIP, HotelIds, OperationType.Insert);
                if (dt != null && dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["HasError"].ToString() == "1")
                    {
    ...
    protected void lbEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (hfUserId > 0)
            {
                DataTable dt = new BAL_User().usp_UserLogin_Get(hfUserId, null,null);
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    ddlRole_SelectedIndexChanged(null, null);
                    txtPhoneNumber.Text = string.IsNullOrEmpty(dt.Rows[0]["PhoneNumber"].ToString()) ? "" : dt.Rows[0]["PhoneNumber"].ToString();
                    
                    hfId.Value = hfUserId.ToString();

                    DataTable dtHotelMapping = new BAL_User().UserHotelMapping_Crud(hfUserId, null);
                    foreach (DataRow dr in dtHotelMapping.Rows)
                    {
                        string HotelId = dr["HotelId"].ToString();
                        ListItem itemHotelIds = chkHotelList.Items.FindByValue(HotelId.ToString());
                        if (chkHotelList.Items.Contains(itemHotelIds))
                            itemHotelIds.Selected = true;
                    }
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Id > 0)
            {
                DataTable dt = new BAL_User().usp_UserLogin_Get(Id,null,null,null,null,null,null,false,UserId,UserIP,null,OperationType.Delete);
                if (dt != null && dt.Rows.Count > 0)
                {
    ...
    private void ResetModalControls()
    {
        ...
        ddlRole_SelectedIndexChanged(null, null);
        txtUserName.Text = txtEmailAddress.Text = txtLoginId.Text = "";
        chk_IsActive.Checked = true;
        hfId.Value = "0";
        txtPhoneNumber.Text = "";
        chkHotelList.ClearSelection();
    }
